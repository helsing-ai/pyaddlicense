...
def test__macro_license__fills_year_and_holder() -> None:
    holder = "example"
    year = 2022
...
def test__macro_license__no_macros_makes_no_changes() -> None:
    license_tpl = "2022 example"
    result = macro_license(license_tpl, "", None)
    assert result == license_tpl


def test__get_license_template__raises_exception_given_both_options(tmp_path: Path) -> None:
    ...
    with pytest.raises(BothLicenseOptionsGivenError):
        get_license_template(_LICENSE_CONTENT_TPL_STR, license_as_file.open())


def test__get_license_template__loads_file(tmp_path: Path) -> None:
    license_as_file = tmp_path / "license.txt"
    license_as_file.write_text(_LICENSE_CONTENT_TPL_FILE)
...
def test__get_license_template__returns_license() -> None:
    result = get_license_template(_LICENSE_CONTENT_TPL_STR, None)
    assert result == _LICENSE_CONTENT_TPL_STR


def test__create_ignorespec_from_gitignore__returns_correct_spec(tmp_path: Path) -> None:
...
def test__comment_license_header__js_example_correct() -> None:
    test_text = "I am a\nblock level\ncomment."
...
def test__comment_license_header__given_just_mid() -> None:
...
def test__contains_license_or_is_generated__has_copyright() -> None:
    s = "# COPYRIGHT SOME COMPANY"
    assert contains_license_or_is_generated(s)


def test__contains_license_or_is_generated__is_autogenerated() -> None:
    s = "# Generated by THE VOID. DO NOT EDIT."
    assert contains_license_or_is_generated(s)


def test__contains_license_or_is_generated__contains_none() -> None:
    s = "Copyriht My Great Lisense Company"  # (Intentional mistakes to fail test)
    assert not contains_license_or_is_generated(s)


def test__file_has_license_or_is_generated__has_copyright(tmp_path: Path) -> None:
    tmp_file = tmp_path / "ohhai.py"
    tmp_file.write_text("# 1990 COPYRIGHT THE NINETIES KIDS")

    assert file_has_license_or_is_generated(tmp_file)


def test__file_has_license_or_is_generated__is_autogenerated(tmp_path: Path) -> None:
    tmp_file = tmp_path / "ohhai.py"
...
def test__file_has_license_or_is_generated__no_copyright_not_autogenerated(tmp_path: Path) -> None:
    tmp_file = tmp_path / "ohhai.py"
    tmp_file.write_text(_EXAMPLE_PYTHON)

    assert not file_has_license_or_is_generated(tmp_file)
...
def test__add_license_to_file__standard_added_at_top(tmp_path: Path) -> None:
    tmp_file = tmp_path / "ohhai.py"
    tmp_file.write_text(_EXAMPLE_PYTHON)
...
def test__add_license_to_file__bash_added_after_hashbang(tmp_path: Path) -> None:
...
def test__process__empty_file(tmp_path: Path) -> None:
...
def test__process__standard_file(tmp_path: Path) -> None:
    settings = get_generic_test_settings()
    settings.root = tmp_path
...
def test__process__new_line_first_thing_file(tmp_path: Path) -> None:
...
def test__process__check_mode_does_count_makes_no_changes(tmp_path: Path) -> None:
    settings = get_generic_test_settings()
    settings.root = tmp_path
    settings.check = True
...
def test__process__process_recurses_into_directories(tmp_path: Path) -> None:
    settings = get_generic_test_settings()
    settings.root = tmp_path
    settings.check = True
